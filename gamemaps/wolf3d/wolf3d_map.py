...
class LevelHeader:
  def __init__(self, name, width, height, plane_offsets, plane_lengths):
    # The name is not really the name of the level but what is what is shown
    # in the original level editor.
    self.name = name
    self.width = width
    self.height = height
...
def map_headers(path):
  ...
  with open(path, 'rb') as reader:
    magic = int.from_bytes(reader.read(2), byteorder='little', signed=False)
    ...
    if tag == near_pointer_tag or tag == far_pointer_tag:
      ...
      if count:
        if tag == near_pointer_tag:
          ...
          # This is applied backwards, so it means repeat count words
          # from the 'value' words before the end.
          #
          # The shifting is converting from words to bytes (multiply by 2).
        ...
        # and the tag is the second byte of the word and the next value
        # is the first byte of the word.
...
def decompress_rlew(data):
  ...
  # For Python using array.array('H', data) would be more efficient instead of
  # doing it this way.
  data = io.BytesIO(data)
