/*
    ...
    sdlExplorer-alt.c bs.c arraylist.c -lSDL -lSDL_ttf -lSDL_image
...
ArrayList * GetDirectoryListing() {
    ...
    hFind = FindFirstFile("F:/Videos/Movies/*.avi", &FFD); /* avi's only */
...
unsigned int renderDirList(SDL_Surface * screen, ArrayList * dirList, TTF_Font * font, unsigned int start, int dirDown) {
    const unsigned int itemHeight = 32;

    const unsigned int baseHeight = 768;
    SDL_Rect sr;
    sr.x = 0;
    sr.y = 90;
    #ifndef NOIMAGES
    ...
    SDL_Rect destFocus = {  10+itemHeight, 90 +(listPosition-start)*itemHeight,400, itemHeight};
    SDL_BlitSurface(surfLF , &surfLF->clip_rect, screen, &destFocus);

    unsigned int maxItems = (baseHeight - 95) / itemHeight;
    if (dirDown > 0) {
        if (start > 0) {            
...
int OnControl(unsigned int id, SDL_Surface * screen) {
    if (id == 1) {
        //printf("MENU: Quit\n");
        SDL_Quit();
        return 1;
...
/*int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd){*/
    ...
    screen = SDL_SetVideoMode(1024, 768, 0, SDL_DOUBLEBUF);
    ...
    /* INIT RESOURCES */
        ...
        surfLF = IMG_Load("images/list-focus.png");
    ...
    SDL_Surface * path = TTF_RenderText_Blended( mainFont, "Path: F:/Videos/Movies/", scWhite );
    ...
    guiButtons[0] = &butQuit;
    guiButtons[1] = &butView;
    guiButtons[2] = &butTool;
    guiButtons[3] = &butHelp;
    
    createGUIButton(&butQuit, 1, 0   , 0, 84, 28, "Quit" , mainFont);
    createGUIButton(&butView, 2, 84  , 0, 84, 28, "View" , mainFont);
    createGUIButton(&butTool, 3, 84*2, 0, 84, 28, "Tools" , mainFont);
    createGUIButton(&butHelp, 4, 84*3, 0, 84, 28, "Help", mainFont);
    createGUIButton(&guiList, 6, 15, 95, 690, 466,"", mainFont);
    
    setnavGUIButton(&butQuit, NULL, &guiList, &butHelp, &butView);
    setnavGUIButton(&butView, NULL, &guiList, &butQuit, &butTool);
    setnavGUIButton(&butTool, NULL, &guiList, &butView, &butHelp);
    setnavGUIButton(&butHelp, NULL, &guiList, &butTool, &butQuit);
    setnavGUIButton(&guiList, &guiList, &guiList, &guiList, &guiList);

    guiList.c.isVisible = false;
    butQuit.c.hasFocus = true;
    curButton = &butQuit;
    ...
    renderGUIButton(&butQuit, screen);
    renderGUIButton(&butView, screen);
    renderGUIButton(&butTool, screen);
    renderGUIButton(&butHelp, screen);
    unsigned int botList = renderDirList(screen, dirList, mainFont, 0, 1);
        ...
        while( SDL_PollEvent( &event ) ) {
            if (event.type == SDL_QUIT) {
                ...
                if (event.key.keysym.sym == SDLK_UP) {
                    if (curButton == &guiList && listPosition > 0) {
                        listPosition --;
                        botList = renderDirList(screen, dirList, mainFont, botList, 0);
                        SDL_Flip(screen);
                    } else {
                        curButton = changeFocus(screen, curButton, curButton->bu);
                    }
                } else if (event.key.keysym.sym == SDLK_DOWN) {
                    if (curButton == &guiList) {
                        listPosition++;
                        botList = renderDirList(screen, dirList, mainFont, botList, 0);
                        SDL_Flip(screen);
                    } else {
                    ...
                    bQuit = OnControl(curButton->c.id,screen);
                } else if (event.key.keysym.sym == SDLK_PAGEDOWN) {
                    if (botList < dirList->length) {
                        botList = renderDirList(screen, dirList, mainFont, botList, 1);
                        SDL_Flip(screen);
                    }                    
                    ...
                    if (event.button.y < 30) { /* This if is only in place to speed up since all buttons are above there */
                        for (i = 0; i < 4; i++) {
                            if (checkPointInRect(event.button.x,event.button.y, guiButtons[i]->c.pos)) {
                                bQuit = OnControl(guiButtons[i]->c.id,screen);
                            }
                        }
                    }
                }
