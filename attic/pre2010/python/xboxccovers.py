...
def getletterLIST(l):
    ...
    #for id in range(0,5):
        ...
        req.add_header("Cookie", "__utma=127001695.517025715.1143448914.1143533796.1143703934.8; __utmz=127001695.1143448914.1.1.utmccn=(referral)|utmcsr=cdcovers.cc|utmcct=/|utmcmd=referral; phpAds_geoInfo=au; __utmb=127001695; __utmc=127001695")
        #Cookie: 

        html2 = urllib2.urlopen(req,txdata).read()
        coverstring = re.search('document.the_cover.src="http://covers.cdcovers.cc/show.php[?](.*)"' + "'", html2)
        print 'http://covers.cdcovers.cc/show.php?' + coverstring.group(1) + "\n"
    ...
    #tf.close()
    thelist = ""
    print "Check C drive fro thelist"
